...
const FALLBACK_TO = "mike@mikethezier.com";
...
function validate(i) {
  ...
  if (!i.tier) return "Something went wrong picking that session. Please email mike@mikethezier.com and he’ll sort it out.";
...
function compose(i, rateCard) {
  ...
  const text =
    ...
    (i.note ? `\n\n---\n\n${i.note}\n` : "\n") +
    `\n---\nChosen from the senior investment guide (${rateCard} rates).\n` +
  ...
  const html =
    ...
    `Chosen from the senior investment guide (${esc(rateCard)} rates).<br>` +
...
async function deliver(env, { subject, text, html, replyTo }) {
  ...
  const res = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      from: env.INQUIRY_FROM || "Senior Sessions <seniors@pilgrimage.media>",
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      await deliver(env, {
        subject: `Senior session chosen — ${i.name} — ${i.label || i.tier}`,
        text,
        html,
      ...
      return reply(
        false,
        "Something went wrong sending that — sorry. Please email mike@mikethezier.com and he’ll pick it straight up.",
        502
      );
    }
