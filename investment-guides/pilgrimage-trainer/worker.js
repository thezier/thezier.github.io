...
function compose(i, rateCard) {
  ...
  const text =
    ...
    (i.note ? `\n\n---\n\n${i.note}\n` : "\n") +
    `\n---\nChosen from the Trainer & Gym guide (${rateCard} rates).\n` +
  ...
  const html =
    ...
    `Chosen from the Trainer & Gym guide (${esc(rateCard)} rates).<br>` +
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      await deliver(env, {
        subject: `Trainer session chosen — ${i.name} — ${i.label || i.tier}`,
        text,
        html,
